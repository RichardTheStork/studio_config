...
class MayaActions(HookBaseClass):
	...
	def _create_reference(self, path, sg_publish_data):
		...
		print "#"*30
		"""
		Create ref with right namespace
		check if workarea is rigging (asset) or shot/sequence!!!
		"""
		# wtd_fw = self.load_framework("tk-framework-wtd_v0.x.x")
		# mayaActions = wtd_fw.import_module("maya.actions")
		...
		currentType = currentContext.entity["type"]
		
		if currentType == "Asset":
			print "Check if it is rig or a set..."
			"""
			Create right namespace
			"""
			namespace = "mod"
			indexNr = 1
			existingRefs = []
			assetName = sg_publish_data.get("entity").get("name")
			print assetName
			tempList = cmds.ls("%s*" %(assetName), recursive = True)
			...
			for existing in tempList:
				...
				if number.find("_") != -1:
					try:
						...
						print "!!!CHECK NAME!!!   could not set %s with '_' to int!" %number
						continue
				else:
					try:
						number = int(number)
					except:
						print "could not set %s to int!" %number
						continue
				existingRefs.append(number)
				
			if len(existingRefs) > 0:
				indexNr = sorted(existingRefs)[-1]+1
				indexNr = ('000' + str(indexNr))[-3:]
				namespace = "%s_%s_%s" % (assetName, namespace, indexNr)
			else:
				
			namespace = "%s_%s" %(resolution, namespace)
			namespace = namespace.replace(" ", "_")
					
		elif currentType == "Sequence" or currentType == "Shot":
			print "Check if it is loaded into a shot or sequence!"
			"""
			Create right namespace
			"""
			...
			namespace = "%s_%s" % (assetName, indexNr)
			namespace = namespace.replace(" ", "_")
					
			self._create_locator_reference("PRP_%s" %namespace)
		else:
			print 'not loading in a shot, sequence or asset...'
			print 'maybe we are in projectlevel???'
	...
	def _create_locator_reference(self, name):
		print 'TODO:'
		print "create locator with name %s" %name
