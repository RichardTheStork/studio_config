...
class PrePublishHook(Hook):
	...
	def execute(self, tasks, work_template, progress_cb, **kwargs):
		...
		for task in tasks:
			...
			# pre-publish positionlist output
			if item["type"] == "poslist":
				for i in item['other_params']:
					print i
				errors.extend(self.__validate_item_for_positionlist_publish(item))
			else:
	...
	def __validate_item_for_positionlist_publish(self, item):
		"""
		Validate that we can make the positionlist with the current items...
		...
		for i in sceneList:
			if len(sceneList[i]) == 0:
				errors.append("No content in %s" %i)
				
		# with open(publish_path, "a") as target:
		for grp in sceneList:
			for o in sceneList[grp]:
				object = sceneList[grp][o]
				assetName = object["asset"]
				asset = self.findAsset(assetName)
				if asset:
					object["asset"] = asset
				else:
					errors.append("No asset found for %s" %o)
	...
	def findAsset(self, assetName):
		...
		if assetName in self.allLoadedAssets:
			asset = self.allLoadedAssets[assetName]
		else:
			asset = self.parent.shotgun.find_one("Asset",filters,fields)
			self.allLoadedAssets[assetName] = asset
		if not asset:
			# print "couldn't find asset"
			return None
		else:
			# print (asset)
			if asset["code"].lower() != assetName.lower():
				# print "FOUND BUT NOT THE SAME %s - %s" %(assetName, asset["code"])
				return None
