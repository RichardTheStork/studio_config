...
class PublishHook(Hook):
	...
	def execute(self, tasks, work_template, comment, thumbnail_path, sg_task, primary_task, primary_publish_path, progress_cb, **kwargs):
		...
		for task in tasks:
			...
			if output["name"] == "poslist":
				try:
					self.__publish_positionlist(item, output, work_template, primary_publish_path, sg_task, comment, thumbnail_path, progress_cb)
				except Exception, e:
					errors.append("Positionlist Failed - %s" %e)
			else:
	...
	def __publish_positionlist(self, item, output, work_template, primary_publish_path, 
										sg_task, comment, thumbnail_path, progress_cb):
		"""
		Publish a positionlist for the scene and publish it to Shotgun.
		
		:param item:                    The item to publish
		:param item["other_params"]:      The (basic) positionlist info for all objects in scene
		...
		print sg_task
		tempTask = self.parent.shotgun.find_one("Task", [['id','is', sg_task['id']]], ["sg_short_name"])
		fields["Task"] = tempTask["sg_short_name"]
		...
		#
		with open(publish_path, "w") as target:
			None
			
		print "TESTING CONTENT OTHER PARAMS"
		sceneList = item["other_params"]
		progress_cb(20, "Analysing scene")		
		timer = 20
		grpAmount = 60/len(sceneList)
		grpNumber = 0
		grpSum = 0
		for grp in sceneList:
			if len(sceneList[grp]) != 0:
				prpAmount = float(grpAmount)/len(sceneList[grp])
				prpNumber = 0
				for o in sceneList[grp]:
					tempgrpSum = grpAmount*grpNumber + prpAmount*prpNumber
					if int(grpSum) < int(tempgrpSum):
						grpSum = tempgrpSum
						progress_cb((timer + grpSum), "Analysing scene - %s" %grp)
					object = sceneList[grp][o]
									
					object["assetType"] = objectTypes[object["assetType"]]
					prpNumber += 1
			grpNumber += 1
		
		progress_cb(80, "Exporting Positionlist")
		with open(publish_path, "w") as target:
			target.write(json.dumps(sceneList, sort_keys=True, indent=4, separators=(',', ': ')))

		# publishedInfo = self.parent.shotgun.find_one("PublishedFile", [["code","is","yqhtfdsjhfshdshgfx"]], ['id','code'])
		# print publishedInfo
		publishedInfo = self.parent.shotgun.find_one("PublishedFile", [["code","is",publish_name]], ['id','code'])
		# print publishedInfo
		
		if publishedInfo != None:
			print "UPDATING EXISTING POSLIST!!!"
			args = {
				...
				# "dependency_paths": [primary_publish_path], # turn on for publishing poslist together with normal publish
				# "published_file_type":tank_type,
				"task": sg_task
			}
			self.parent.shotgun.update("PublishedFile", publishedInfo["id"], args)
		else:
			print "CREATING NEW POSLIST..."
			args = {
				"tk": self.parent.tank,
				"context": self.parent.context,
				"comment": comment,
